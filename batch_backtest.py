...
STOCKS = [
    # 第一梯队
    ("第一梯队", "600900", "长江电力", "cdp_daily.csv"),
    ("第一梯队", "600377", "宁沪高速", "600377_daily.csv"),
    # 第二梯队
    ("第二梯队", "601006", "大秦铁路", "601006_daily.csv"),
    ("第二梯队", "600012", "皖通高速", "600012_daily.csv"),
    # 第三梯队
    ("第三梯队", "601088", "中国神华", "601088_daily.csv"),
    ("第三梯队", "601398", "工商银行", "601398_daily.csv"),
]
    ...
    for line in lines:
        ...
        if not in_result:
            ...
            if line.startswith('[时间范围]'):
                # 最近 N 年: 2010-01-01 至 2026-12-31
                parts = line.split()
                if len(parts) >= 5:
...
def main():
    ...
    # Markdown 表格
    header = (
        f"| 梯队 | 代码 | 名称 | 总收益率 | 年化收益率 | 夏普比率 | 最大回撤 | "
        f"交易次数 | 胜率 | 数据周期 |\n"
        f"|:----:|:----:|:----|---------:|-----------:|---------:|---------:|---------:|-------:|:----------|\n"
    )
    print(header)
    ...
    print()
    print("-"*80)
    print(f"策略参数: 布林带周期={BB_PERIOD}, 标准差={BB_DEV}, 固定止损={STOP_LOSS:.0%}")
    print(f"初始资金: {INITIAL_CASH:.0f}, 佣金: 0.1%")
    print()

    # 保存汇总结果
    with open("batch_backtest_result.md", "w", encoding="utf-8") as f:
        f.write("# 均值回归策略批量回测结果\n\n")
        f.write(f"策略参数: 布林带周期={BB_PERIOD}, 标准差={BB_DEV}, 固定止损={STOP_LOSS:.0%}\n\n")
        f.write(f"初始资金: {INITIAL_CASH:.0f}, 佣金: 0.1%\n\n")
        f.writelines(md_lines)

    print(f"汇总结果已保存到 batch_backtest_result.md")
